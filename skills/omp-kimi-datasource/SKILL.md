...
本 plugin 后面挂了 7 个外部数据源。每一行的"数据源名"就是你后续传给 `get_data_source_desc(name=...)` 的参数。
...
1. 法律法规 / 司法案例 → `yuandian_law`
2. `get_data_source_desc(name="yuandian_law")`
3. 从返回文档里选择法规查询、案例搜索等最匹配的 API
4. `call_data_source_tool("yuandian_law", "<文档里写的 api>", {...})`

不要凭记忆写 `api_name` 或参数名；法律数据源也必须先读 desc。
...
绝大部分数据源 API 把完整结果以 CSV 形式写到 `file_path`（部分接口文档也可能写作 `filepath`）。漏传会报 `Missing required parameters: file_path`。不知道传啥时，给一个 `/tmp/<场景>_<时间戳>.csv` 即可。
...
- 如果返回文本中出现 `⚠️ 警告：请求中指定了 file_path=...，但未观察到该路径的文件已落盘`，说明没有文件生成，不要继续声称"完整数据已落盘"，直接基于 `data_preview` 回答
  - `file_path` 和 `filepath` 都会触发同样的落盘检查
- 返回末尾形如 `[kimi-datasource] request-id: ... · tool-call-id: ...` 或 `[kimi-datasource] tool-call-id: ...` 的 trace 行只用于后端排障关联，不是数据结果，不要把它当作查询结论
...
- 回答用户时使用用户提问时使用的语言；用户用中文问就用中文答，用户用英文问就用英文答
