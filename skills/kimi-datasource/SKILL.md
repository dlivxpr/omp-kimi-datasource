...
description: |
  通用数据源助手。当用户要查股票/财报/技术指标/全球宏观经济/中国企业工商/学术论文 这类外部数据时，使用这个 skill。
  本 plugin 注册了两个工具：
...
2. **完整数据已落盘**：你在 `params` 里传的 `file_path` 所指的 CSV 已由工具写到磁盘（返回文本中也会回显路径），不再受 50 行限制

策略：
- 用户只问"XX 现在多少钱"、"中国 2023 GDP 多少"这种单值 → `data_preview` 一般够，直接答
- 用户要画图、对比、算盈亏、列清单，或结果超过 50 行 → 用 `read` 工具把落盘的 CSV 读出来再处理
- 混合 A+港股查询时服务端会把结果拆成 `_a.csv` / `_hk.csv`（工具按服务端实际返回的文件名落盘），原 `file_path` 那个文件不会生成
